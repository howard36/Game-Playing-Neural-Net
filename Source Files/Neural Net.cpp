...
void printBoardC4(Vec s, bool isState) {
    if (isState) {
        ...
        if (s.rows() != maxMoves + 1) {
            cout << "Error: probability distribution size does not match when "
                    "printing\n";
            return;
        }
        for (int i = 0; i < 7; i++)
    ...
    do {
        ...
        while (true) {
            if (turn == 1) { // Human Turn
                ...
                if (move == -1) {
                    if (history.size() == 1)
                        cout << "Cannot undo move! This is the starting "
                                "position\n";
                    else {
                        history.pop_back();
            ...
            if (pair.first) {
                if (pair.second == 1) {
                    ...
                    cout << "Tie\n";
                printBoardHex(turn * state,
                              true); // show final state from your point of view
                break;
            }
            state = -state;
            turn = -turn;
